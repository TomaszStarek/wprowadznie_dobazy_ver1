...
namespace WindowsFormsApp1
{
    public partial class Form4 : Form
    {
        ...
        private void potwierdzenia_minut()
        {
            ...
            dobazy("OP010", opis_awarii);
        }


        private void pobieranie()
        {
            string cmdString = "SELECT ([opis]) FROM [dbo].[awarieGM3] WHERE [stacja] = 'OP10'  GROUP BY ([opis]) ORDER BY COUNT(*) DESC OFFSET 0 ROWS FETCH NEXT 3 ROWS ONLY;";    //OFFSET 3 ROWS FETCH NEXT 3 ROWS ONLY
                                                                                                                                                                                    //  string connString = @"database=PLKWIM0P21B1GM3; Initial Catalog = test; User ID = loginsql; Password = Jabil12345";           //server=192.168.1.10; 
                                                                                                                                                                                    //  string connString = @"database=PLKWIM0P21B1GM3; Initial Catalog = test; Integrated Security = SSPI";
